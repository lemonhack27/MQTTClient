...
static int led_gpio_init(struct device_node *nd) {
    ...
    if (!gpio_is_valid(leddev.gpio_led)) {
        printk(KERN_ERR "leddev: invalid\n");
        return -EINVAL;
    }

    /* 申请使用GPIO */
    ret = gpio_request(leddev.gpio_led, "LED0");   /* LED0给gpio取名字 */
    if (ret != 0) {
...
/*
 * @description         : platform驱动的probe函数，
 *                        当驱动与设备匹配以后，此函数就会执行
...
static int led_probe(struct platform_device *pdev) {
    ...
    printk("led_probe running\n");

    /* 初始化LED */
    ret = led_gpio_init(pdev->dev.of_node); /* of_node：associated device tree node*/
    if (ret < 0) return ret;

    /* 1、设置设备号 */
    ret = alloc_chrdev_region(&leddev.devid, 0, LEDDEV_CNT, LEDDEV_NAME);
    if (ret < 0) {
        pr_err("%s: alloc_chrdev_region failed, ret=%d", LEDDEV_NAME, ret);
        goto free_gpio;
    }
...
static struct platform_driver led_driver = {
    .driver = {
        .name = "stm32mp1-led",         /* 驱动名字，用于和设备匹配（如果无设备树） */
